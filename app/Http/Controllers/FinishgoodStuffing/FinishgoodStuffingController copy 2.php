...
class FinishgoodStuffingController extends Controller
{
    ...
    public function inputPackingGlobal(Request $request)
    {
        ...
        return view('menu.finishgood-stuffing.input-global', [
            'po'           => $po,
            'op'           => $op,
            'poref'        => $poref,
            'mif'          => $mif,
    ...
    public function listDetailGlobal(Request $request)
    {
        ...
        foreach ($shipRows as $sr) {
            $shipInfoByPackpk[$sr->packpk] = [
                ...
                'returning' => (int) $sr->fca === 2, // fca=2: sedang dikembalikan, menunggu diterima FG/Stuffing
                'popk'      => $sr->popk,
                'part'      => $sr->part,
            ];
        }
        ...
        if ($status !== '' && $status !== null) {
            if (in_array($status, ['shipped', 'inspect', 'returning'], true)) {
                // Filter berdasarkan status SHIP (bukan status Pack biasa).
    ...
    public function bulkShipAction(Request $request)
    {
        ...
        if ($action === 'inspect') {
            ...
            // TAHAP 1 dari alur 2-tahap "Kembalikan ke Stuffing" (dipicu dari
            // halaman Inspection). fca: 1 -> 2 ("menunggu diterima FG/Stuffing")
            ...
            $label   = $isReject ? 'Kembalikan ke Stuffing (Reject)' : 'Kembalikan ke Stuffing (Menunggu Diterima)';
            $updated = $db->table('ship')
                ->whereIn('packpk', $ids)
                ->where('fca', 1)
                ->update(['fca' => 2]);
            ...
            // TAHAP 2 dari alur 2-tahap (dipicu dari halaman FG/Stuffing, tombol
            // "Terima Carton"). fca: 2 -> null, DAN 'kembali' BARU diisi sekarang.
            // pack.reject TIDAK diubah di sini -- status Reject/tidak-nya TETAP
            // dipertahankan apa adanya, supaya FG/Stuffing tahu carton mana yang
