...
class FinishgoodStuffingController extends Controller
{
    ...
    public function listDetailGlobal(Request $request)
    {
        ...
        $shipRows = $db->table('ship')
            ->whereIn('popk', $popks)
            ->get(['packpk', 'popk', 'part', 'status', 'fca', 'pinjam', 'kembali']);
        ...
        foreach ($shipRows as $sr) {
            $shipInfoByPackpk[$sr->packpk] = [
                ...
                'ever_inspected' => !empty($sr->pinjam), // BARU -- true selamanya walau fca sudah balik null
                'kembali'        => $sr->kembali ?? null, // BARU -- tanggal diterima balik (kalau sudah)
                'pinjam'         => $sr->pinjam ?? null,  // BARU -- tanggal masuk inspect
                'popk'           => $sr->popk,
                'part'           => $sr->part,
            ];
        }
        ...
        foreach ($rowsByCartonForStatus as $cartonKey => $groupRows) {
            ...
            // Status ship gabungan per carton -- prioritas Shipped > Returning >
            // Inspect (carton yang sedang dikembalikan lebih "urgent" ditindaklanjuti
            // FG/Stuffing daripada sekadar Inspect biasa).
        ...
        $status = $request->input('status'); // '' | planned | packing | complete | sealed | shipped | inspect | returning

        $finalQuery = clone $baseQuery;
        if ($status !== '' && $status !== null) {
            if ($status === 'inspect_history') {
                // BARU: carton yang PERNAH masuk Inspect (pinjam terisi), APA PUN
                // status fca-nya SEKARANG (termasuk yang sudah diterima balik/fca=null).
        ...
        // ============================================================
        // TAHAP 2 -- paginasi per CARTON, bukan per baris.
        //
        // PENTING: $finalQuery->get() di bawah adalah QUERY BARU ke DB --
        // hasilnya ($allFinalRows) adalah OBJECT PHP YANG BERBEDA dari
        // $allMatchingRows, walau isi datanya sama. Property ship_shipped/
        // ship_inspect/ship_returning yang sudah ditempel ke $allMatchingRows
        // TIDAK IKUT ke $allFinalRows -- makanya nanti di loop final ($data)
        // WAJIB lookup ULANG dari $shipInfoByPackpk, BUKAN diasumsikan sudah ada.
        // ============================================================
        ...
        if (isset($sortMap[$sort])) {
            ...
            // Urutan DEFAULT -- No Carton A-Z (SAMA seperti 'carton_asc'),
            // BUKAN lagi secsz+material.
        ...
        foreach ($data as $index => $row) {
            ...
            // FIX UTAMA: lookup ULANG ship info -- $row ini object BARU dari
            // $allFinalRows, BUKAN object yang sama dengan $allMatchingRows.
            ...
            $row->ship_pinjam    = $shipInfo['pinjam'] ?? null;   // BARU -- kapan masuk inspect
            $row->ship_kembali   = $shipInfo['kembali'] ?? null;  // BARU -- kapan diterima balik (null kalau belum)
    ...
    public function bulkShipAction(Request $request)
    {
        ...
        if ($action === 'inspect') {
            ...
            // TAHAP 1 dari alur 2-tahap "Kembalikan ke FinishGood" (dipicu dari
            // halaman Inspection). fca: 1 -> 2 ("menunggu diterima FG/FinishGood")
            // -- SAMA untuk kasus OK maupun Reject. Kolom 'kembali' SENGAJA
            // belum disentuh -- baru diisi di TAHAP 2 (accept_return).
            $isReject = (int) $request->input('reject', 0) === 1;

            $label   = $isReject ? 'Kembalikan ke FinishGood (Reject)' : 'Kembalikan ke FinishGood (Menunggu Diterima)';
            $updated = $db->table('ship')
                ->whereIn('packpk', $ids)
                ->where('fca', 1)
                ->update(['fca' => 2]);

            // BARU: tandai/bersihkan pack.reject BERDASARKAN pilihan user --
            // dieksekusi terlepas dari $updated (supaya reject tetap konsisten
            // ter-set walau baris ship-nya sudah 2 karena double klik, dsb),
            // tapi HANYA untuk packpk yang memang ada di request.
            $db->table('pack')
                ->whereIn('packpk', $ids)
                ->update(['reject' => $isReject ? 1 : null]);
        } else { // 'accept_return'
    ...
    public function partSummaryGlobal(Request $request)
    {
        ...
        $rows = $db->table('pack')
            ...
            ->whereNotNull('part')
            ->where('part', '!=', 0)
            ->select('packpk', 'carton', 'part', 'popk')
            ->get();
        ...
        $packpks = $rows->pluck('packpk')->unique()->values()->all();
        ...
        //  tanggal plan shipment per Part -- kolom po.ship{part}, SAMA
        // pola dengan $shipDateByPopkPart di listDetailGlobal(). Ambil dari
        // popk pertama yang muncul di tiap Part (representatif).
        $shipDateColumns = collect(range(1, 10))->map(fn($i) => "ship{$i}")->all();
        $poShipDateRows = $db->table('po')
            ->whereIn('popk', $popks)
            ->get(array_merge(['popk'], $shipDateColumns));
        $shipDateByPopk = $poShipDateRows->keyBy('popk');
    
        $parts = [];
        foreach ($rows->groupBy('part') as $part => $partRows) {
            $cartonGroups = $partRows->groupBy('carton');
            $total = $cartonGroups->count();
    
            $shipped = $cartonGroups->filter(function ($cartonRows) use ($shipRowsMap) {
                return $cartonRows->contains(function ($r) use ($shipRowsMap) {
                    $status = (int) ($shipRowsMap[$r->packpk]->status ?? 0);
                    return in_array($status, [6, 7], true);
                });
            })->count();
    
            $locked = $cartonGroups->filter(function ($cartonRows) use ($shipRowsMap) {
                return $cartonRows->every(function ($r) use ($shipRowsMap) {
                    $status = (int) ($shipRowsMap[$r->packpk]->status ?? 0);
                    return $status === 7;
                });
            })->count();
    
            //  ambil ship_date dari popk representatif (yang pertama
            // muncul) di Part ini.
            $repPopk  = $partRows->first()->popk;
            $poRow    = $shipDateByPopk->get($repPopk);
            $shipDate = $poRow ? ($poRow->{"ship{$part}"} ?? null) : null;
        ...
        usort($parts, fn($a, $b) => $a['part'] <=> $b['part']);
