...
class SegelPackingController extends Controller
{
    ...
    // TAMBAHKAN method-method ini ke SegelPackingController (class yang
    // sudah ada) -- reuse postToInventory()/resolveJkarton()/self::LPB_CONNECTION
    // yang SUDAH ADA, tidak perlu ditulis ulang.

    /**
     * BARU -- endpoint yang dipanggil EXIM (server-to-server) saat
     * lockship di-set jadi 1. TIDAK melakukan apa pun ke tabel ship
     * (EXIM sudah set status=7 sendiri lewat toggleLock()) -- endpoint
     * ini KHUSUS mem-posting ke sistem inventory (opit/bj/bjo di LPB),
     * yaitu bagian yang dulu dikerjakan processComplete()/processPartial()
     * SELAIN bagian pack/ship-nya.
     *
     * Payload yang diharapkan dari EXIM:
     *   {
     *     "exportpk": 4,
     *     "exdate":   "2026-08-26",   // dipakai sbg actual_shipment / shipdate2
     *     "packpks":  [85207022, 85207023, ...]
     *   }
     *
     * TIDAK ADA session/user login di sini -- 'part' dibaca dari kolom
     * pack.part yang SUDAH ADA (bukan dipilih user), dan 'mif' dideteksi
     * dari koneksi mana packpk itu ditemukan (coba mysql dulu, fallback
     * mysql_andon -- SAMA pola findLo()/findOutsisa() di modul lain).
     */
    public function handleEximLockShipment(Request $request)
    {
        $validated = $request->validate([
        ...
        $packpks = collect($validated['packpks'])->unique()->values();

        // FIX UTAMA -- cari packpk di KEDUA koneksi, EXIM tidak kirim mif.
        $foundRows = collect();
        foreach (['mysql' => 2, 'mysql_andon' => 1] as $connName => $mifGuess) {
            $rows = DB::connection($connName)->table('pack')->whereIn('packpk', $packpks)->get();
            foreach ($rows as $r) {
                $r->_connection = $connName;
                $r->_mif = $mifGuess;
                $foundRows->push($r);
            }
        }
        ...
        if ($foundRows->isEmpty()) {
            return response()->json([
                'success' => false,
                'message' => 'Tidak ada carton (packpk) yang cocok di sistem Packing.',
            ], 404);
        }
        ...
        // Group per (koneksi + popk + part) -- SATU dokumen OPIT/bj/bjo per
        // kombinasi ini (SAMA granularitas dgn processComplete/processPartial
        // versi lama, cuma sekarang 'part' dibaca, bukan dipilih user).
        $groups = $foundRows->groupBy(fn ($r) => $r->_connection . '|' . $r->popk . '|' . ($r->part ?: '0'));

        foreach ($groups as $key => $rowsInGroup) {
            [$connName, $popkStr, $part] = explode('|', $key, 3);
            $popk = (int) $popkStr;
            $mif  = $rowsInGroup->first()->_mif;
        
            if ($part === '0' || $part === '') {
                $errors[] = "popk {$popk}: carton belum punya 'part' -- dilewati (tidak tahu ini Complete atau Partial keberapa).";
                continue;
            }
        
            try {
                $shipped = $this->processEximLock($popk, $part, $exdate, $connName, $mif, $rowsInGroup);
                if ($shipped) {
                    $groupPackpks = $rowsInGroup->pluck('packpk')->values();
                    DB::connection($connName)->table('ship')
                        ->whereIn('packpk', $groupPackpks)
                        ->update(['status' => 7]);
                }
        
                $results[] = "popk {$popk} part {$part}: " . ($shipped ? 'berhasil posting ke inventory' : 'dilewati (qty 0)');
            } catch (\Throwable $e) {
                Log::critical('handleEximLockShipment: gagal posting ke inventory', [
                    'exportpk' => $validated['exportpk'],
                    'popk'     => $popk,
                    'part'     => $part,
                    'error'    => $e->getMessage(),
                ]);
                $errors[] = "popk {$popk} part {$part}: gagal ({$e->getMessage()})";
            }
        }
        ...
        return response()->json([
            ...
            'message' => empty($errors)
                ? 'Semua carton berhasil di-posting ke sistem inventory.'
                : 'Sebagian carton gagal diposting, cek detail di errors[].',
            'results' => $results,
            'errors'  => $errors,
        ], empty($errors) ? 200 : 207); // 207 = sebagian berhasil
    }

    /**
     * BARU -- versi ringkas dari processComplete()/processPartial(): TIDAK
     * menyentuh pack.status atau insert baris ship SAMA SEKALI (itu sudah
     * jadi tanggung jawab alur packing/EXIM sebelumnya), CUMA melakukan
     * posting ke opit/bj/bjo (postToInventory()) + update po.shipdate2 dan
     * po.ship{part}.
     */
    private function processEximLock(int $popk, string $part, string $exdate, string $connection, int $mif, $packRowsForGroup): bool
    {
        $po = DB::connection($connection)->table('po')->where('popk', $popk)->first();
        abort_unless($po, 404, "PO dengan popk {$popk} tidak ditemukan.");

        // FIX UTAMA -- actual_shipment diambil dari export.exdate (dikirim
        // EXIM), BUKAN input manual user seperti alur lama.
        DB::connection($connection)->table('po')->where('popk', $popk)->update([
            'shipdate2' => $exdate,
        ]);
        $po->shipdate2 = $exdate; // sinkronkan object lokal, dipakai postToInventory()

        $qtyship = (int) $packRowsForGroup->sum('pcs');
        if ($qtyship <= 0) {
            return false;
        }

        $jml = $packRowsForGroup->filter(fn ($r) => (int) $r->pcs > 0)->count();
        $jkarton = $this->resolveJkarton($po->gabung, $jml);

        // FIX UTAMA -- ctx (userpk/bj_column/bj_base) TIDAK BISA dari
        // session (EXIM tidak login sebagai user Finishing manapun),
        // di-resolve dari mif hasil deteksi packpk tadi.
        $ctx = $this->shipperContextFromMif($mif);

        DB::connection(self::LPB_CONNECTION)->transaction(function () use ($po, $qtyship, $jkarton, $popk, $part, $ctx) {
            $this->postToInventory($po, $qtyship, $jkarton, $popk, $part, $ctx); // REUSE method yang sudah ada, TIDAK diubah
        });

        // Tandai po.ship{N} (atau ship10 kalau part='10' = Complete) --
        // SAMA konvensi legacy (dulu di-set di processComplete()/processPartial()).
        DB::connection($connection)->table('po')->where('popk', $popk)->update([
            "ship{$part}" => $exdate,
        ]);
    ...
    /**
     * BARU -- pengganti shipperContext() versi session-based, KHUSUS
     * dipakai jalur EXIM (tidak ada session sama sekali). Mapping SAMA
     * PERSIS dgn shipperContext() yang sudah ada.
     */
    private function shipperContextFromMif(int $mif): array
    {
        switch ($mif) {
            case 1:
                return ['userpk' => 62, 'bj_column' => 'bjd1', 'bj_base' => 1000000];
            case 2:
                return ['userpk' => 63, 'bj_column' => 'bjd2', 'bj_base' => 2000000];
            default:
                abort(422, "MIF tidak dikenali: {$mif}");
        }
    }

    public function verifyLpbPosting(Request $request)
    {
        $validated = $request->validate([
        ...
        $popks = collect($validated['popks'])->unique()->values();
        ...
        foreach ($popks as $popk) {
            // Cari popk ini di KEDUA koneksi (mysql / mysql_andon) -- SAMA
            // pola dengan handleEximLockShipment(), supaya tahu PO/OP-nya
            // utk dicocokkan ke opit.
            $poRow = null;
            $foundConnection = null;
            foreach (['mysql', 'mysql_andon'] as $connName) {
                $row = DB::connection($connName)->table('po')
                    ->join('pack', 'pack.popk', '=', 'po.popk')
                    ->where('pack.popk', $popk)
                    ->select('po.popk', 'po.POno', 'po.OP', 'po.ship1', 'po.ship2', 'po.ship3', 'po.ship4', 'po.ship5',
                        'po.ship6', 'po.ship7', 'po.ship8', 'po.ship9', 'po.ship10')
                    ->first();
                if ($row) {
                    $poRow = $row;
                    $foundConnection = $connName;
                    break;
                }
            }
            ...
            if (!$poRow) {
                $result[] = [
                    'popk'     => $popk,
                    'verified' => false,
                    'reason'   => 'popk tidak ditemukan di sistem Packing (mysql maupun mysql_andon).',
                ];
                continue;
            }
    
            // Cek baris bj -- SUMBER KEBENARAN utama "sudah ke-posting apa
            // belum" (setiap posting SELALU insert 1 baris bj baru).
            $bjRows = $lpb->table('bj')
                ->where('popk', $popk)
                ->when($part, fn ($q) => $q->where('part', $part))
                ->orderByDesc('bjpk')
                ->get(['bjpk', 'opitpk', 'nobukti', 'tgl', 'qty', 'part', 'ctn', 'shipdate', 'posting']);
            ...
            if ($bjRows->isEmpty()) {
                $result[] = [
                    ...
                    'verified'   => false,
                    'reason'     => $part
                        ? "Tidak ada baris 'bj' untuk popk {$popk} part {$part} -- BELUM ke-posting ke LPB."
                        : "Tidak ada baris 'bj' sama sekali untuk popk {$popk} -- BELUM PERNAH ke-posting ke LPB.",
                ];
                continue;
            }
    
            // Ambil opit terkait (dari opitpk di bj -- lebih akurat daripada
            // re-lookup via nopo/noop, karena bj SUDAH tahu persis opitpk-nya).
            $opitpks = $bjRows->pluck('opitpk')->unique()->values();
            $opitRows = $lpb->table('opit')
                ->whereIn('opitpk', $opitpks)
                ->get(['opitpk', 'opitid', 'nopo', 'noop', 'qtyship', 'part']);
    
            // Ambil bjo terkait (per tgldok = bj.shipdate).
            $shipdates = $bjRows->pluck('shipdate')->unique()->values();
            $bjoRows = $lpb->table('bjo')
                ->whereIn('opitpk', $opitpks)
                ->whereIn('tgldok', $shipdates)
                ->get(['bjopk', 'opitpk', 'nobukti', 'tglkirim', 'tgldok', 'qty', 'jkarton', 'part']);
            ...
            $result[] = [
                ...
                'OP'         => $poRow->OP,
                'connection' => $foundConnection,
                'verified'   => true, // ada minimal 1 baris bj -- dianggap sudah ke-posting
                'bj'         => $bjRows,
                'opit'       => $opitRows,
                'bjo'        => $bjoRows,
            ];
        ...
        $allVerified = collect($result)->every(fn ($r) => $r['verified'] === true);
