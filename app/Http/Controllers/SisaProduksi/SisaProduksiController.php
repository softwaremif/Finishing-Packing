...
class SisaProduksiController extends Controller
{
    ...
    public function detailList($popk, Request $request)
    {
        ...
        // ============================================================
        // TAB 'keluar' -- SEKARANG SUMBER DATANYA outsisa/outsisadt (BUKAN
        // lagi bj.pcsk). TIDAK ADA aksi apa pun -- keluar barang cuma lewat
        // menu Keluarkan Sisa.
        // ============================================================
        if ($tab === 'keluar') {
            $lines = $db->table('outsisadt')
                ...
                ->where('bj.popk', $popk)
                ->select(
                    'outsisadt.outpk', 'outsisadt.size', 'outsisadt.color', 'outsisadt.secsz', 'outsisadt.qty',
                    'outsisa.tglout', 'outsisa.penerima', 'outsisa.keterangan',
                    'outsisa.stsapv1', 'outsisa.stsapv2', 'outsisa.stsapv3',
                    'bj.grade'
                )
                ->get();
    
            $grouped = $lines->groupBy('outpk')->map(function ($group) {
                $first = $group->first();
                return (object) [
                    'outpk'        => $first->outpk,
                    'tglout'       => $first->tglout,
                    ...
                    'grade'        => $first->grade,
                    'sizes'        => $group->map(fn ($l) => ['label' => $l->size, 'qty' => (float) $l->qty])->values(),
                    'pcs'          => $group->sum('qty'),
                    'status_label' => $this->resolveOutStatusLabel($first),
                    'stsapv1'      => (int) $first->stsapv1 === 1 ? 1 : ((int) $first->stsapv1 === 0 ? 0 : null), // BARU
                    'stsapv2'      => (int) $first->stsapv2 === 1 ? 1 : ((int) $first->stsapv2 === 0 ? 0 : null), // BARU
                    'stsapv3'      => (int) $first->stsapv3 === 1 ? 1 : ((int) $first->stsapv3 === 0 ? 0 : null), // BARU
                ];
            })->sortByDesc('tglout')->values();
    
            $total = $grouped->count();
            $data  = $grouped->slice($offset, $rows)->values();
            foreach ($data as $i => $row) {
                $row->no = $offset + $i + 1;
            }
        ...
        // ============================================================
        // TAB 'pending' / 'received' -- SAMA seperti sebelumnya, TAPI
        // SEKARANG di-JOIN ke lodt+lo supaya tahu nomor LO & status
        // approval-nya per bjpk.
        // ============================================================
        ...
        // BARU -- attach info LO (nomor + status approve) per bjpk.
        $bjpks = $bjRows->pluck('bjpk')->filter()->values();
        $loInfoByBjpk = collect();
        if ($bjpks->isNotEmpty()) {
            $loInfoByBjpk = $db->table('lodt')
                ->join('lo', 'lo.lopk', '=', 'lodt.lopk')
                ->whereIn('lodt.bjpk', $bjpks)
                ->select('lodt.bjpk', 'lo.lopk', 'lo.stsapv1', 'lo.stsapv2', 'lo.stsapv3')
                ->get()
                ->keyBy('bjpk');
        }
    
        foreach ($bjRows as $r) {
            $loInfo = $loInfoByBjpk->get($r->bjpk);
            $r->lo_lopk    = $loInfo->lopk ?? null;
            $r->lo_approved = $loInfo
                ? ((int) $loInfo->stsapv1 === 1 && (int) $loInfo->stsapv2 === 1 && (int) $loInfo->stsapv3 === 1)
                : false;
        }
    
        $outputAggRows = $tab === 'received' ? $outputAggRows : collect();
        foreach ($outputAggRows as $r) {
            $r->lo_lopk = null;
            $r->lo_approved = false;
        }
    ...
    // BARU -- helper status label utk outsisa (SAMA pola resolveStatusLabel
    // di LoController, tapi disalin di sini karena controller berbeda).
    private function resolveOutStatusLabel($out): string
    {
        $isRejected = fn($val) => $val !== null && (int) $val === 0;
        if ($isRejected($out->stsapv1)) return 'Ditolak (Level 1)';
        if ($isRejected($out->stsapv2)) return 'Ditolak (Level 2)';
        if ($isRejected($out->stsapv3)) return 'Ditolak (Level 3)';
        if ((int) $out->stsapv3 === 1) return 'Selesai (Approved)';
        if ((int) $out->stsapv2 === 1) return 'Menunggu Approve Level 3';
        if ((int) $out->stsapv1 === 1) return 'Menunggu Approve Level 2';
        return 'Menunggu Approve Level 1';
    }
