...
class PackingController extends Controller
{
    ...
    public function getList(Request $request)
    {
        ...
        if ($isSuper) {
            ...
            $merged = $rowsAndon->concat($rowsMysql)
                ->sortByDesc('OP')
                ->values();

            $total = $merged->count();
            $data  = $merged->slice($offset, $rows)->values();
        } else {
            $mif        = session('pos') == 1 ? 1 : 2;
            $connection = session('pos') == 1 ? 'mysql_andon' : 'mysql';

            $all   = $this->fetchAll($connection, $mif, $request);
            $total = $all->count();
            $data  = $all->slice($offset, $rows)->values();
        }
        ...
        foreach ($data as $i => $row) {
            ...
            $row->ctn_balance          = (int) ($row->packing_ctn - $row->ctn);
        }
    ...
    private function fetchAll(string $connection, int $mif, Request $request)
    {
        ...
        $this->applyListFilters($res, $request);
