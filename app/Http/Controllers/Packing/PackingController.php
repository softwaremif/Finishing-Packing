...
class PackingController extends Controller
{
        ...
        return view('menu.shared.packing-input-global', array_merge($existingData, [
            'pageConfig' => [
                ...
                'routes' => [
                    ...
                    'distinctDimensiCtn'     => route('packing.distinct-dimensi-ctn'),   // BARU
                    'bulkUpdateDimensiCtn'   => route('packing.bulk-update-dimensi-ctn'), // BARU
                    ...
                    'modalBulkDimensiCtn'    => 'menu.packing.modal-bulk-dimensi-ctn', // BARU
                ],
            ],
        ]));
    }
    ...
    public function listDetailGlobal(Request $request)
    {
        ...
        foreach ($shipRows as $sr) {
            $shipInfoByPackpk[$sr->packpk] = [
                ...
                'returning' => (int) $sr->fca === 2, // BARU -- FIX UTAMA
                'popk'      => $sr->popk,
                'part'      => $sr->part,
            ];
        }
        ...
        if (isset($sortMap[$sort])) {
            ...
            $cartonKeys = $cartonKeys->sort(function ($a, $b) use ($representativeByCarton, $sortColumn, $sortDir) {
                ...
                // BARU -- FIX UTAMA: strnatcmp() bukan strcmp(), khusus utk kolom
    ...
    public function storeGlobal(Request $request)
    {
        ...
        try {
            ...
            if ($removedPackpks->isNotEmpty()) {
                ...
                $db->table('ship')->whereIn('packpk', $removedPackpks)->delete(); // BARU -- FIX UTAMA (permintaan sebelumnya)
            }
            ...
            if (!$editMode && $useAutoSplit && $isSingleComboSingleSize) {
                ...
                // Preload SEMUA carton & nobar yang sudah dipakai di PO/OP ini --
                // dipakai untuk cek bentrok secara instan tanpa query berulang di
                // dalam loop.
                ...
                $cartonOffset = 0;
                $nobarOffset  = 0;
                $safetyLimit  = 10000; // jaga-jaga supaya tidak infinite loop kalau data korup
                
                for ($n = 0; $n < $totalToGenerate; $n++) {
                    // Cari nomor CARTON bebas berikutnya -- offset 0 = $nocar ITU SENDIRI
                    // (angka yang diketik user), lompat ke offset berikutnya HANYA kalau
                    // bentrok.
                    $guard = 0;
                    do {
                    ...
                    // Cari nomor BARCODE bebas berikutnya -- SAMA pola, offset 0 = $nobar
                    // ITU SENDIRI (B0003 tetap B0003 kalau belum dipakai), BUKAN direset
                    // ke 0001 lagi.
                    $candidateNobar = '';
                    if ($nobar !== '') {
                        $guard = 0;
                ...
                // Safety net -- seharusnya TIDAK PERNAH tercapai lagi karena loop di atas
                // sudah aktif mencari nomor bebas, tapi dipertahankan sebagai jaring
                // pengaman kalau ada race condition (dua user submit bersamaan).
                $generatedCartons = array_column($generatedRows, 'carton');
            ...
            foreach ($grouped as $lines) {
                ...
                if ($packpk > 0) {
                    ...
                    $shipRowData = $rowData;
                    unset($shipRowData['reject']);
                    unset($shipRowData['panjang'], $shipRowData['lebar'], $shipRowData['tinggi']);
                
                    $db->table('ship')->where('packpk', $packpk)->update($shipRowData);
                    $updatedRows++;
                } else {
    ...
    public function updateCtnGlobal(Request $request)
    {
        ...
        try {
            ...
            foreach ($orderedPacks as $x) {
                ...
                $db->table('pack')->where('packpk', $x->packpk)->update($upd);
                $db->table('ship')->where('packpk', $x->packpk)->update($upd); 
                $this->insertActpackHistory($x->packpk, $x, $newQtyArr);
                $jumlahUpdate++;
    ...
    public function deleteActualGlobal(Request $request)
    {
        ...
        try {
            ...
            foreach ($ids as $id) {
                ...
                $db->table('ship')->where('packpk', $id)->update($update);
                $this->insertActpackHistory($id, $pack, $newQtyArr);
                $jumlah++;
            }
        ...
        try {
            ...
            if ($mode === 'carton') {
                ...
                foreach ($naturalOrder as $oldCarton) {
                    ...
                    foreach ($rowsInGroup as $row) {
                        ...
                        $db->table('ship')->where('packpk', $row->packpk)->update($updateData);
                    }
                    $offset++;
                }
            ...
            for ($idx = $startIndex; $idx < count($sortedCartonKeys); $idx++) {
                ...
                foreach ($rowsInGroup as $row) {
                    ...
                    $db->table('ship')->where('packpk', $row->packpk)->update(['nobar' => $newNobar]); // BARU -- FIX UTAMA
                }
                $offset++;
            }
    ...
    /**
     * GANTI distinctDimensiCtn() -- TAMBAH 'weights' (kombinasi NW/GW yang
     * sudah pernah dipakai), terpisah dari 'combos' (Panjang/Lebar/Tinggi).
     */
    public function distinctDimensiCtn(Request $request)
    {
        ...
        // BARU -- FIX UTAMA: kombinasi NW/GW yang sudah pernah dipakai
        // (terpisah dari dimensi, karena kolomnya juga ada di 'ship').
    ...
    /**
     * GANTI bulkUpdateDimensiCtn() -- TAMBAH nw/gw (opsional). Dimensi
     * (panjang/lebar/tinggi) tetap CUMA update 'pack', TAPI nw/gw update
     * KEDUANYA ('pack' DAN 'ship') karena kolomnya memang ada di dua-duanya.
     */
    public function bulkUpdateDimensiCtn(Request $request)
    {
        ...
        // BARU -- FIX UTAMA: pisah update dimensi (khusus 'pack') dan
        // update berat (KEDUA tabel) -- masing-masing HANYA di-update kalau
        // memang diisi (supaya bisa isi salah satu saja, dimensi ATAU berat).
        ...
        if (!empty($weightUpdate)) {
            ...
            $db->table('ship')->whereIn('packpk', $packpks)->update($weightUpdate); // nw/gw ADA di ship
        }
