...
class PackingController extends Controller
{
    ...
    //     // ============================================================
    //     // Lookup ship LEBIH AWAL -- dibutuhkan utk hitung statusCounts &
    //     // filter status='inspect'/'shipped' di bawah. Ship di koneksi
    //     // PER-MIF ($db), SAMA seperti pack/po. $shipInfoByPackpkGlobal ini
    //     // adalah SATU-SATUNYA sumber kebenaran ship info -- dipakai lagi
    //     // nanti di loop final ($data), BUKAN mengandalkan property yang
    //     // "menempel" ke object $allMatchingRows (object PHP tidak persist
    //     // lintas query, harus di-lookup ulang dari array ini).
    //     // ============================================================
    //     $allMatchingPackpks = $allMatchingRows->pluck('packpk')->filter()->unique()->values()->all();

    //     $shipInfoByPackpkGlobal = [];
    //     if (!empty($allMatchingPackpks)) {
    //         $shipRowsGlobal = $db->table('ship')
    //             ->whereIn('packpk', $allMatchingPackpks)
    //             ->select('packpk', 'status', 'fca', 'tanggal')
    //             ->get();

    //         foreach ($shipRowsGlobal as $sr) {
    //             $shipInfoByPackpkGlobal[$sr->packpk] = [
    ...
    //         $info = $shipInfoByPackpkGlobal[$r->packpk] ?? null;
    //         $r->ship_shipped = $info['shipped'] ?? false;
    //         $r->ship_inspect = $info['inspect'] ?? false;
    //         $r->ship_date    = $info['date'] ?? null;
    //     }
    ...
    //         // Status ship gabungan per carton -- prioritas Shipped > Inspect,
    //         // SAMA pola dengan getShipStampForGroup() di JS.
    //         $anyShipped = $groupRows->contains(fn ($r) => $r->ship_shipped === true);
    //         $anyInspect = $groupRows->contains(fn ($r) => $r->ship_inspect === true);
    //         $cartonShipStatusMap[$cartonKey] = $anyShipped ? 'shipped' : ($anyInspect ? 'inspect' : null);
    //     }
    ...
    //     $status = $request->input('status'); // '' | 'planned' | 'packing' | 'complete' | 'sealed' | 'inspect' | 'shipped'

    //     // Filter status -- SAMA pola, berbasis NOMOR CARTON. Branch ke
    //     // cartonShipStatusMap kalau status='inspect'/'shipped'.
    //     $finalQuery = clone $baseQuery;
    //     if ($status !== '' && $status !== null) {
    //         if (in_array($status, ['inspect', 'shipped'], true)) {
    ...
    //     // ============================================================
    //     // TAHAP 2 -- paginasi per CARTON, bukan per baris. Ambil SEMUA
    //     // baris yang lolos filter (tanpa limit), kelompokkan per carton,
    //     // urutkan CARTON-nya (bukan barisnya), baru slice sesuai halaman.
    //     //
    ...
    //     // ship_inspect/ship_date yang sudah ditempel ke $allMatchingRows
    //     // TIDAK IKUT ke $allFinalRows -- makanya nanti di loop final
    //     // ($data) harus lookup ULANG dari $shipInfoByPackpkGlobal, BUKAN
    ...
    //         // jadi property ship_shipped/ship_inspect/ship_date TIDAK
    //         // otomatis ikut. Harus di-lookup lagi dari array yang sama.
    //         $info = $shipInfoByPackpkGlobal[$row->packpk] ?? null;
    //         $row->ship_shipped = $info['shipped'] ?? false;
    //         $row->ship_inspect = $info['inspect'] ?? false;
    //         $row->ship_date    = $info['date'] ?? null;
    //     }

    //     return response()->json([
    //         // 'total' SEKARANG jumlah CARTON (dipakai JS untuk hitung
    //         // maxPage: Math.ceil(total / rows)) -- BUKAN lagi jumlah baris.
    //         'total'         => $totalCarton,
    ...
    public function listDetailGlobal(Request $request)
    {
        ...
        $matchingCartonQuery = $db->table('pack')
            ...
            ->when($color, fn($q) => $q->where('material', $color))
            ->when($secsz, fn($q) => $q->where('secsz', $secsz))
            ->when($part, fn($q) => $q->where('part', $part)) // BARU
            ->when($search, function ($q) use ($search) {
                $q->where(function ($x) use ($search) {
                    $x->where('nobar', 'like', "%{$search}%")
                        ->orWhere('carton', 'like', "%{$search}%");
        ...
        // BARU: tempel status ship LEBIH AWAL -- dibutuhkan untuk hitung
        // statusCounts & filter status='shipped'/'inspect' di bawah.
        foreach ($allMatchingRows as $r) {
        ...
        $cartonStatusMap     = [];
        $cartonShipStatusMap = []; // BARU
        foreach ($rowsByCartonForStatus as $cartonKey => $groupRows) {
            $cartonStatusMap[$cartonKey] = $this->getPackGroupStatus($groupRows);
        
            // BARU: status ship gabungan per carton -- prioritas Shipped > Inspect,
            // SAMA pola dengan getShipStampForGroup() di JS.
            $anyShipped = $groupRows->contains(fn ($r) => $r->ship_shipped === true);
            $anyInspect = $groupRows->contains(fn ($r) => $r->ship_inspect === true);
            $cartonShipStatusMap[$cartonKey] = $anyShipped ? 'shipped' : ($anyInspect ? 'inspect' : null);
        ...
        $statusCounts = [
            ...
            'complete' => count(array_filter($cartonStatusMap, fn($s) => $s === 'complete')),
            'sealed'   => count(array_filter($cartonStatusMap, fn($s) => $s === 'sealed')),
            'shipped'  => count(array_filter($cartonShipStatusMap, fn($s) => $s === 'shipped')), // BARU
            'inspect'  => count(array_filter($cartonShipStatusMap, fn($s) => $s === 'inspect')), // BARU
        ];
        
        $status = $request->input('status'); // '' | 'planned' | 'packing' | 'complete' | 'sealed' | 'shipped' | 'inspect'
        
        $finalQuery = clone $baseQuery;
        if ($status !== '' && $status !== null) {
            if (in_array($status, ['shipped', 'inspect'], true)) {
                // BARU: filter berdasarkan status SHIP (bukan status Pack biasa).
                $matchingCartons = array_keys(array_filter($cartonShipStatusMap, fn($s) => $s === $status));
            } else {
                $matchingCartons = array_keys(array_filter($cartonStatusMap, fn($s) => $s === $status));
            }
