...
class PackingController extends Controller
{
    ...
    public function getList(Request $request)
    {
        ...
        $countQuery = DB::table('bj')
            ->join('po', 'po.popk', '=', 'bj.popk')
            ->leftJoinSub($transfer, 'trf', function ($join) {
                $join->on('bj.popk', '=', 'trf.popk');
            })
            ->where('bj.check2', 0)
            ->where('po.qty', '>', 0)
            ->where('po.OP', '<>', '')
        ...
        $this->applyListFilters($countQuery, $request);

        $total = $countQuery->distinct()->count('po.popk');
        ...
        // Packing ACTUAL (pcs sudah terisi actual)
        $packing = DB::table('pack')
            ->selectRaw('popk, SUM(pcs) as packing_qty')
            ->groupBy('popk');

        // Packing PLAN (pcsp = rencana qty per carton, terlepas dari actual-nya)
        ...
        // Agregasi status segel per popk, dari tabel pack yang sudah diproses (status=5)
        $segel = DB::table('pack')
            ->selectRaw("
                popk,
        ...
        $query = DB::table('bj')
            ...
            ->leftJoinSub($checked, 'chk', function ($join) {
                $join->on('bj.popk', '=', 'chk.popk');
            })
            ->leftJoinSub($packing, 'pk', function ($join) {
                $join->on('bj.popk', '=', 'pk.popk');
            })
            ->leftJoinSub($packingPlan, 'pkp', function ($join) {
                $join->on('bj.popk', '=', 'pkp.popk');
            })
            ->leftJoinSub($packingCtn, 'pctn', function ($join) {
                $join->on('bj.popk', '=', 'pctn.popk');
            })
            ->leftJoinSub($packStatus, 'pst', function ($join) {
                $join->on('bj.popk', '=', 'pst.popk');
            })
            ...
            ->leftJoinSub($segel, 'sgl', function ($join) {
                $join->on('bj.popk', '=', 'sgl.popk');
            })
            ...
            ->selectRaw("
                ...
                -- Packing Pcs: Plan, Actual, Balance (Actual - Plan)
        ...
        foreach ($data as $i => $row) {
            ...
            $row->segel_complete       = (int) $row->segel_complete;
            $row->packing_qty_plan     = (int) $row->packing_qty_plan;
            $row->packing_qty          = (int) $row->packing_qty;
            $row->packing_qty_balance  = (int) $row->packing_qty_balance;
            $row->packing_ctn          = (int) $row->packing_ctn;
            $row->ctn_balance          = (int) ($row->packing_ctn - $row->ctn); // CTN Actual - CTN Plan
        }

        $summary = [
            'qty'                  => $data->sum('qty'),
            'transfer'             => $data->sum('transfer'),
            'checked_qty'          => $data->sum('checked_qty'),
    ...
    {
        $breakdown = $this->getBreakdownData($popk);
        extract($breakdown);
        ...
        $lines = DB::table('line')
            ->where('mif', session('pos'))->whereNull('stsbar')
            ->orderBy('linenm')->get();
    ...
    private function getBreakdownData($popk)
    {
        ...
        // Satu query ini mencakup ACTUAL (qty{i}, pcs) dan PLAN (qtyp{i}, pcsp)
        // dari tabel pack.
        $dt3 = DB::table('pack')
            ->selectRaw(
        ...
        $planQty        = [];
        $transQty       = [];
        $diffTransQty   = []; // Balance Transfer = Transfer - Order
        $diffPackQty    = []; // Balance Pack     = Actual  - Plan

        for ($i = 1; $i <= 40; $i++) {
            $orderQty[$i]     = $dt2->{"qty$i"} ?? 0;
            $readyQty[$i]     = $dt3->{"qty$i"} ?? 0;   // ACTUAL pack qty
            $planQty[$i]      = $dt3->{"qtyp$i"} ?? 0;  // PLAN pack qty
        ...
        // Total Balance Transfer = total Transfer - total Order
        $totDiffTrans = ($summary->pcs ?? 0) - $totOrder;

        // Total Balance Pack = total Actual - total Plan
    ...
    public function breakdownSummary($popk)
    {
        return view('menu.packing.partials.breakdown_summary', $this->getBreakdownData($popk));
    }
                ...
                if ($nocar > 0) {
                    if ($totalp2 > 1) {
                        ...
                        if ((int) $request->check == 1 && $dtpoQtyCol > 0) {
                            ...
                            $cartonStart = $nocar;
                            for ($i = $cartonStart; $i <= ($cartonStart + $kali - 1); $i++) {
                                $nobarFull = $nobarBase . sprintf('%04d', $i);
                            ...
                            if ($sisa > 0) {
                                $cartonSisa  = $cartonStart + $kali;
                                $nobarSisa   = $nobarBase . sprintf('%04d', $cartonSisa);
        ...
        try {
            ...
            foreach ($packs as $pack) {
                ...
                DB::table('pack')
                    ...
                    ->update([
                        'carton' => $no,
                        'nobar'  => str_pad($no, 4, '0', STR_PAD_LEFT)
                    ]);
    ...
    public function copyMultiple(Request $request)
    {
        ...
        try {
            ...
            foreach ($rows as $row) {
                ...
                DB::table('pack')
                    ->where('popk', $popk)
                    ->where('carton', '>', $row->carton)
                    ->increment('carton', $copies);
            ...
            $prefix = '';
            if ($packs->count()) {
                $prefix = substr($packs->first()->nobar, 0, -4);
            }

            $no = 1;
            foreach ($packs as $p) {
                DB::table('pack')
                    ->where('packpk', $p->packpk)
                    ->update([
                        'carton' => $no,
                        'nobar'  => $prefix . sprintf('%04d', $no),
                    ]);
                $no++;
            }
            ...
            if ($totalActualDitolak > 0) {
                $namaSizeHabis = DB::table('po')->where('popk', $popk)->first();
                $labelSize = collect(array_keys($sizeHabisInfo))
                    ->map(fn($i) => $namaSizeHabis->{"size{$i}"} ?? "Size {$i}")
                    ->implode(', ');
    ...
    public function urutCtn(Request $request)
    {
        ...
        try {
            ...
            foreach ($packs as $pack) {
                ...
                $nobarNew  = $nobarBase . sprintf('%04d', $k);
