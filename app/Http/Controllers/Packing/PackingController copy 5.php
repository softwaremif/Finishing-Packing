...
class PackingController extends Controller
{
    ...
    private function aggregateByPoOp($collection)
    {
        return $collection
            ...
            ->map(function ($group) {
                ...
                // BARU: simpan daftar popk pembentuk grup ini -- field internal,
                // dibuang lagi sebelum dikirim ke JSON (lihat getList()).
    ...
    public function inputPackingGlobal(Request $request)
    {
        ...
        $colorSecszCombos = collect($groups)->map(function ($g) {
            return [
                'material' => $g['material'],
                'secsz'    => $g['secsz'],
                'orderQty' => $g['orderQty'],
    ...
    private function getBreakdownDataGlobal(?string $po, string $op, ?string $poref, int $mif, string $connection): array
    {
        ...
        $ctnSummary = [
            ...
            'ctnActual'  => $ctnActualTotal,
            'ctnBalance' => $ctnActualTotal - $ctnPlanTotal,
        ];
    ...
    public function listDetailGlobal(Request $request)
    {
        ...
        $popks = $db->table('po')
            ...
            // FIX (baru): filter poref juga, konsisten dengan fetchAll()/
            // getBreakdownDataGlobal().
            ->when(
                $poref !== null && $poref !== '',
                fn($q) => $q->where('poref', $poref),
                fn($q) => $q->where(function ($qq) {
        ...
        $cr     = $request->cr;
        $size   = $request->size;
        $color  = $request->color; // filter tambahan khusus Global (per Color)
        $secsz  = $request->secsz;
        $page   = (int) $request->input('page', 1);
        $rows   = (int) $request->input('rows', 50);
        $search = trim($request->search ?? '');

        // FIX UTAMA: whereIn('popk', $popks) -- pengganti where('popk', $popk)
        // tunggal. `pack.material`/`pack.secsz` sudah ada di tabel pack sendiri
        // (hasil sync dari store()), jadi TIDAK perlu JOIN ke po lagi.
        $query = $db->table('pack')
            ->whereIn('popk', $popks)
            ->when($cr, fn($q) => $q->where('carton', $cr))
            ->when($size, fn($q) => $q->where("qtyp{$size}", '>', 0))
            ->when($color, fn($q) => $q->where('material', $color))
            ->when($secsz, fn($q) => $q->where('secsz', $secsz)) // BARU
            ->when($search, function ($q) use ($search) {
                $q->where(function ($x) use ($search) {
                    $x->where('nobar', 'like', "%{$search}%")
                        ->orWhere('carton', 'like', "%{$search}%");
        ...
        $allMatchingRows = (clone $query)->get();
        ...
        // Terapkan filter status -- SEKARANG berbasis NOMOR CARTON yang statusnya
        // (gabungan) sudah cocok, bukan filter baris individual lagi.
        if ($status !== '' && $status !== null) {
            $matchingCartons = array_keys(array_filter($cartonStatusMap, fn($s) => $s === $status));

            // Kalau tidak ada carton yang cocok, paksa hasil kosong (whereIn
            // dengan array kosong akan error di beberapa versi Laravel, jadi
            // pakai placeholder yang pasti tidak ada).
            $query->whereIn('carton', !empty($matchingCartons) ? $matchingCartons : ['__NONE__']);
        }

        $total = $query->count();
        $totalCartonDistinct = (clone $query)->distinct()->count('carton');
        ...
        if (isset($sortMap[$sort])) {
            ...
            $dataQuery->orderBy($sortColumn, $sortDir);
        } else {
            // Urutan DEFAULT (lama) -- dipertahankan kalau user tidak memilih
            // sort spesifik apa pun.
            $dataQuery->orderBy('secsz')->orderBy('material');
        }
        ...
        return response()->json([
            'total'        => $total,
            'total_carton' => $totalCartonDistinct,
    ...
    public function storeGlobal(Request $request)
    {
        ...
        try {
            ...
            $grouped = collect($breakdown)->groupBy(fn($b) => ($b['material'] ?? '') . '|' . ($b['secsz'] ?? ''));
    ...
    public function deleteActualGlobal(Request $request)
    {
        ...
        try {
            ...
            foreach ($ids as $id) {
                ...
                // BARU: TIDAK ada ->where('popk', $popk) -- packpk yang
                // dipilih bisa berasal dari popk mana pun (Mixed carton).
                $pack = DB::table('pack')
                    ->where('packpk', $id)
                    ->where('status', 4)
                    ->first();
                ...
                // Histori actpack -- SAMA persis pola deleteActual() lama,
                // delta negatif untuk size yang benar-benar punya Actual
                // sebelumnya.
                $sizeIndexesToClear = ($size !== '' && $size !== null) ? [(int) $size] : range(1, 40);
    ...
    public function updateSegelStatusGlobal(Request $request)
    {
        ...
        try {
            ...
            // BARU: TIDAK ada ->where('popk', $popk) -- packpk yang dipilih
            // bisa berasal dari popk mana pun (Mixed carton / lintas Global).
            $packs = DB::table('pack')
                ->whereIn('packpk', $ids)
                ->lockForUpdate()
                ->get();
            ...
            // Catat tanggal/waktu perubahan status segel -- sama pola dengan
            // updateSegelStatus() lama.
            $updateData = ['segel' => $target];
    ...
    public function combosGlobal(Request $request)
    {
        ...
        return response()->json($colorSecszCombos);
    }
}
