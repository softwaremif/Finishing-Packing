...
@section('js_custom')
    ...
    <script>
        ...
        function renderPackingCards(rows) {
            ...
            // BARU: setelah DOM dibangun ulang, tandai lagi carton yang
            ...
            // BARU: refresh sticky bar berdasarkan seleksi gabungan (halaman ini
        ...
        function computePackingGroupData(groupRows) {
            ...
            const partBadgeHtml = partValue ?
                `<span class="badge-soft" style="background:#ede9fe;color:#6d28d9;border-color:#ddd6fe;">
                    ${String(partValue) === '10' ? 'Part 10 &middot; Complete' : 'Part ' + partValue}
                </span>` :
                '';
        ...
        function buildPackingCard(groupRows) {
            ...
            if (!window.canManageSegel) {
                // tidak punya akses -- tampilkan status APA ADANYA,
                // tanpa tombol yang bisa diklik.
                if (d.isSealed) {
        ...
        function onPackingItemClick(e, itemEl) {
            ...
            if (!wasSelected) {
                // BARU: simpan row-row terkait ke cache SEBELUM validasi, supaya
        ...
        function initFilterPartGlobal() {
            ...
            for (let i = 1; i <= 10; i++) {
                partData.push({
                    value: String(i),
                    text: (i === 10) ? 'Part 10 (Complete)' : `Part ${i}`
                });
            }
    ...
    <script>
        ...
        function updateSelectionGlobal() {
            ...
            if (uniqueParts.size > 1) {
                abortSelection('Tidak bisa memilih carton dengan Part berbeda secara bersamaan.');
                return;
            }
