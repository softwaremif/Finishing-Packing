...
@section('content')
{{-- Halaman detail INSPECTION -- GLOBAL MURNI (sama seperti Finished
     Goods): mencakup SELURUH baris dalam satu PO+OP, lintas semua
     place/color/secsz/part. Bedanya dari Finished Goods:
     - TIDAK ada input/scan barcode
     - hanya carton ABU-ABU (fca = 1) yang bisa dipilih
     - menu biru hanya berisi "Kembalikan ke Stuffing" --}}
<style>
    :root {
    ...
    /* Di halaman inspection HANYA abu-abu (fca=1) yang bisa dipilih */
...
@endphp
{{-- Menu biru: hanya "Kembalikan ke Stuffing" --}}
<div id="stickTopBar" class="sticky-order-bar">
    <div class="sticky-order-inner">
        <div>
            ...
            <span id="inspFeedback" style="margin-left:14px; font-weight:600;"></span>
        </div>
        <div>
...
<div class="container-fluid py-4 px-4">
    ...
    <x-details.header
        title="Inspection"
        :back-url="route('inspection.index')"
    />
    ...
    {{-- ===================== BREAKDOWN SIZE & QTY (GLOBAL) ===================== --}}
    <div class="ftitle">Detail Packing / Carton</div>
    {{-- <table class="print-table">
        <tr style="background:#cbd5e1;">
            <th width="10%">Color</th>
            <th width="8%">Sec Size</th>
    ...
    {{-- TIDAK ada blok Scan Nobar di halaman inspection --}}
...
<script>
(function () {
    var bulkUrl = "{{ route('inspection.bulk') }}";
    var csrf    = "{{ csrf_token() }}";
    var pono    = "{{ $pono }}";
    var op      = "{{ $op }}";
    var busy    = false;
    ...
    var feedbackEl  = document.getElementById('inspFeedback');
    ...
    function showFeedback(ok, msg) {
        ...
        feedbackEl.style.color = ok ? '#d1fae5' : '#fecaca';
    }
    ...
    /* ============ MULTI-SELECT: hanya carton ABU-ABU (global) ============ */
    ...
    // Klik badge carton -> toggle pilihan; HANYA grup 'gray' (fca = 1)
    document.addEventListener('click', function (e) {
        var el = e.target;
        if (!el.classList || !el.classList.contains('carton-input')) return;

        if (el.dataset.group !== 'gray') return; // hijau/biru/none diabaikan
    ...
    btnStuffing.addEventListener('click', function () {
        var cartons = selectedCartons();
        ...
        postJson(bulkUrl, { pono: pono, op: op, action: 'stuffing', cartons: cartons })
        .then(function (r) {
