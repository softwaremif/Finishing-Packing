...
@section('js_custom')
    ...
    <script>
        ...
        function computePackingGroupData(groupRows) {
            ...
            groupRows.forEach(function(row) {
                ...
                // BARU: label material row ini, SUDAH termasuk penanda duplikat.
            ...
            if (uniqueCombos.size === 1) {
                // BARU: pakai getComboLabel() juga, bukan first.material mentah.
        ...
        function buildPackingCard(groupRows) {
            ...
            const editButtonHtml = d.isSealed ?
                '' :
                `<i class="fas fa-pen icon-btn" title="Edit" onclick="event.stopPropagation(); editCartonGlobal('${d.packpksAttr}')"></i>`;

            // FIX UTAMA: kalau sudah Sealed DAN hasPart (sudah masuk proses
            // shipment), JANGAN tampilkan tombol Unseal -- ganti jadi indikator
            // biasa saja (tidak bisa diklik).
            ...
            if (d.isSealed && d.hasPart) {
                ...
                actionButtonHtml = `<button class="btn ${d.canSeal ? 'btn-dark' : 'btn-outline-secondary'}" ${d.canSeal ? '' : 'disabled'}
        ...
        function buildPackingListRow(groupRows) {
            ...
            // FIX UTAMA: SAMA seperti buildPackingCard() -- sembunyikan Unseal
            // kalau sudah Sealed DAN hasPart.
            let actionButtonHtml;
            if (d.isSealed && d.hasPart) {
                ...
                actionButtonHtml = `<button class="btn ${d.canSeal ? 'btn-dark' : 'btn-outline-secondary'} btn-sm" ${d.canSeal ? '' : 'disabled'}
            ...
            return `
                <tr class="packing-select-item packing-list-row" data-packpks="${d.packpksAttr}" data-sealed="${d.isSealed ? 1 : 0}" data-haspart="${d.hasPart ? 1 : 0}"
                    ...
                    <td class="text-start" style="font-size:12.5px; color:#475569;">
                        ${d.first.nobar ? d.first.nobar : '<span class="text-muted">-</span>'}
                    </td>
    ...
    <script>
        ...
        function updateSelectionGlobal() {
            ...
            if (hasSegel) {
                // ============================================================
                // FIX UTAMA: kalau ADA baris terpilih yang kolom `part`-nya
                // TIDAK null (sudah masuk proses shipment), JANGAN tampilkan
                // "Buka Segel" di sticky bar.
                // ============================================================
                const hasPart = selectedRows.some(r => r.part !== null && r.part !== undefined && r.part !== '' && r
                    .part !== 0);
