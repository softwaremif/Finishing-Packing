...
@section('css_custom')
    <style>
        ...
        /* ============================================================
           SAMA dengan index Packing -- cell gabungan Order Info & PO No,
           kolom Ex Factory, ringkasan CTN 1-kolom.
           ============================================================ */
        ...
        /* Wrap teks kolom PO No -- SAMA fix dengan index Packing. */
...
@section('content')
    <div class="page-wrap">
        ...
        >
            <x-slot name="filters">
                <input data-dg-filter="packing_plan_status" data-dg-filter-type="select"
                    data-dg-chip-label="Status Packing List"
                    data-dg-options='[
                        {"value":"","text":"Semua Status Packing List"},
            ...
            <table id="dgOrder" class="easyui-datagrid" style="width:100%;height:600px" url="{{ route('packing.list') }}?fin=1"
                ...
                {{-- Kolom Aksi s/d Ex Factory dibekukan (frozen), SAMA
                     pola dengan index Packing. --}}
                <thead frozen="true">
                    <tr>
...
@section('js_custom')
    <script>
        ...
        // ============================================================
        // FILTER STATE (index) -- tambah exFactory & planStatus, TETAP
        // simpan modalPo/modalOp/modalMif (khusus halaman ini, karena
        // ada modal Segel/Status Packing yang perlu di-restore juga).
        // ============================================================
        ...
        function savePackingNavState() {
            ...
            let state = {
                ...
                planStatus: $('#dgOrder_filterbar [data-dg-filter="packing_plan_status"]').combobox('getValue'),
                page: pageNumber,
        ...
        // Status Packing Plan -- badge Pending/Partial/Complete.
        ...
        // Ringkasan CTN -- 1 kolom, stacked bar + legend.
        function formatCtnSummary(value, row) {
            ...
            const segments = [
                { label: 'Sealed',  value: sealed,     color: '#1e293b' },
                { label: 'Full',    value: full,        color: '#22c55e' },
                { label: 'Partial', value: partial,     color: '#3b82f6' },
                { label: 'Plan',    value: notStarted,  color: '#cbd5e1' },
            ];

            const barHtml = segments
                .filter(s => s.value > 0)
                .map(s => `<span class="ctn-seg" style="width:${pct(s.value)}%;background:${s.color};" title="${s.label}: ${s.value}"></span>`)
                .join('');

            const legendHtml = segments
                .filter(s => s.value > 0)
                .map(s => `<span class="ctn-legend-item"><span class="ctn-dot" style="background:${s.color};"></span>${s.value} ${s.label}</span>`)
                .join('');
        ...
        // ============================================================
        // MODAL: buka rincian PO+OP -- TIDAK DIUBAH.
        // ============================================================
        ...
        // ============================================================
        // KOLOM AKSI DI DALAM MODAL — TIDAK DIUBAH.
        // ============================================================
        ...
        // ============================================================
        // onLoadSuccess MODAL — TIDAK DIUBAH.
        // ============================================================
        function onPackingDetailLoad(data) {
            ...
            $('#packingModalDesc').text(rows.length ? (rows[0].silhouette ?? '-') : '-');
        ...
        // ============================================================
        // SELEKSI CHECKBOX DI DALAM MODAL -- TIDAK DIUBAH.
        // ============================================================
        ...
        // ============================================================
        // MODAL: Input Status Packing -- TIDAK DIUBAH.
        // ============================================================
        ...
        // ============================================================
        // MODAL: Segel Packing -- TIDAK DIUBAH.
        // ============================================================
