...
@section('css_custom')
    <style>
        ...
        .packing-card .badge-status.packing { background:#dbeafe; color:#1d4ed8; }
        .packing-card .badge-status.complete { background:#dcfce7; color:#15803d; }
        .packing-card .badge-status.sealed { background:#e2e8f0; color:#334155; }
        ...
        .packing-card .progress-main .bar { height: 100%; border-radius: 3px; }
        ...
        .packing-card .size-row .dot {
            width: 8px; height: 8px; border-radius: 50%; background:#0f172a; flex-shrink:0;
        }
        .packing-card .size-row .name { flex: 0 0 auto; min-width: 110px; color:#334155; }
        .packing-card .size-row .mini-progress {
            flex: 1; height: 5px; background:#eef0f2; border-radius:3px; overflow:hidden;
        }
        ...
        .packing-card .card-actions { display:flex; gap:8px; margin-top:12px; }
            ...
            <div class="card-body p-0">
                ...
                <div class="p-3">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <button class="btn btn-sm btn-outline-secondary" id="btnPackingCardsPrev" onclick="packingCardsGoPage(-1)">
                                <i class="fas fa-chevron-left"></i>
                            </button>
                            <span style="font-size:12.5px;" id="packingCardsPageLabel"></span>
                            <button class="btn btn-sm btn-outline-secondary" id="btnPackingCardsNext" onclick="packingCardsGoPage(1)">
...
<script>
    ...
    function renderPackingCards(rows) {
        ...
        if (!rows.length) {
            ...
            rows.forEach(function (row) {
                grid.append(buildPackingCard(row));
            });
        }
 
        const maxPage = Math.max(1, Math.ceil(packingCardsTotal / packingCardsRows));
        $('#packingCardsInfo').text(packingCardsTotal + ' carton');
    ...
    function getCartonStatus(row) {
        if (Number(row.segel) === 1) return { key: 'sealed', label: 'Sealed' };
        if (isRowComplete(row)) return { key: 'complete', label: 'Complete' };
        if (Number(row.pcs) > 0) return { key: 'packing', label: 'Packing' };
    ...
    function buildPackingCard(row) {
        const activeIdx = Object.keys(window.activeSizesGlobal || {});
        const plannedSizes = activeIdx.filter(i => Number(row[`qtyp${i}`] || 0) > 0);
        const compositionLabel = plannedSizes.length > 1 ? 'Mixed' : (plannedSizes.length === 1 ? 'Single' : '-');
 
        const status = getCartonStatus(row);
        const totalPlan   = Number(row.pcsp || 0);
        const totalActual = Number(row.pcs || 0);
        ...
        plannedSizes.forEach(function (i) {
            ...
            sizeRows += `
                <div class="size-row">
                    <span class="dot"></span>
                    <span class="name">${row.material ?? '-'} &middot; ${window.activeSizesGlobal[i] ?? i}</span>
        ...
        const secszLabel = row.secsz ? ` &middot; Sec Size ${row.secsz}` : '';
            ...
            <div class="col-12 col-md-6 col-xl-4">
                <div class="packing-card">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <span class="ctn-code">${row.carton ?? '-'}</span>
                            <span class="badge-soft">${compositionLabel}</span>
                            <span class="badge-status ${status.key}">${status.label}</span>
                        </div>
                        <div class="d-flex gap-2">
                            <i class="fas fa-pen icon-btn" title="Edit" onclick="showToast('info','Edit carton (mode Global) belum tersedia.')"></i>
                            <i class="fas fa-copy icon-btn" title="Copy" onclick="showToast('info','Copy carton (mode Global) belum tersedia.')"></i>
                            <i class="fas fa-trash icon-btn" title="Delete" onclick="showToast('info','Delete carton (mode Global) belum tersedia.')"></i>
                        </div>
                    </div>
 
                    <div class="subline mb-1">${row.material ?? '-'}${secszLabel}</div>
                    ...
                    <div class="mb-2">${sizeRows}</div>
 
                    <div class="card-actions">
                        <button class="btn btn-outline-secondary" ${canScan ? '' : 'disabled'}
                            onclick="showToast('info','Fitur Scan belum tersedia.')">Scan +1</button>
                        <button class="btn ${canMark ? 'btn-dark' : 'btn-outline-secondary'}" ${canMark ? '' : 'disabled'}
                            onclick="markCartonPacked(${row.packpk})">Mark packed</button>
                        <button class="btn ${canSeal ? 'btn-dark' : 'btn-outline-secondary'}" ${canSeal ? '' : 'disabled'}
                            onclick="sealCarton(${row.packpk})">Seal</button>
                    </div>
                </div>
            </div>
        `;
    ...
    // Mark packed = Actual disamakan dengan Plan untuk carton ini saja,
    // reuse endpoint packing.update-ctn (sama seperti bulkActualCtn),
    // cuma scope-nya 1 packpk.
    function markCartonPacked(packpk) {
        $.ajax({
            url: "{{ route('packing.update-ctn') }}",
            method: 'POST',
            data: { popk: '', size: '', packpk: String(packpk) },
    ...
    // Seal = reuse endpoint packing.update-segel target=1, scope 1 packpk.
    function sealCarton(packpk) {
        $.ajax({
            url: "{{ route('packing.update-segel') }}",
            method: 'POST',
            data: { popk: '', packpk: String(packpk), target: 1 },
            success: function (res) {
                showToast(res.icon, res.title);
                loadPackingCards();
                reloadBreakdownSummary();
    ...
    function initFilterColorGlobal() {
        ...
        (window.colorListGlobal || []).forEach(function (c) { colorData.push({ value: c, text: c }); });
        $('#filterColorGlobal').combobox({
            data: colorData, valueField: 'value', textField: 'text', value: '',
    ...
    function initFilterSizeGlobalCombobox() {
        ...
        $('#filterSizeGlobal').combobox({
            data: sizeData, valueField: 'value', textField: 'text', value: '',
            editable: false, panelHeight: 'auto',
            onChange: function () { reloadPackingGlobal(); }
        });
    }
    ...
    function initFilterSecszGlobal() {
        ...
        (window.secszListGlobal || []).forEach(function (s) { secszData.push({ value: s, text: s }); });
        $('#filterSecszGlobal').combobox({
            data: secszData, valueField: 'value', textField: 'text', value: '',
            editable: false, panelHeight: 'auto',
            onChange: function () { reloadPackingGlobal(); }
        });
    }
