    ...
    <div class="container-fluid py-4 px-4">
        ...
        <div class="card border-0 shadow-sm rounded-3 mb-4">
            ...
            <div class="card-body p-0" id="dgTransferWrapper">
                ...
                <div id="tabPanePending">
                    <table id="dgPending" class="easyui-datagrid" style="width:100%;height:480px"
                        ...
                        data-options="
                            queryParams: {cr: '{{ $cr }}', tab: 'pending'},
                            loadMsg: 'Memuat data...',
                            onLoadSuccess: onLoadTablePending,
                        ...
                        <thead>
                            <tr>
                                <th field="action" rowspan="2" width="90" align="center" formatter="formatActionPending">Aksi</th>
                                <th field="no" rowspan="2" width="50" align="center" formatter="formatNo">No.</th>
                                ...
                                <th colspan="{{ count($activeSizes) }}" align="center">
                                    <strong>Size</strong>
                                    @if (!empty($dt->secsz)) ({{ $dt->secsz }}) @endif
                                </th>
                                <th field="pcs" rowspan="2" width="90" align="center" formatter="formatTotal">Total<br>(Pcs)</th>
                                <th field="grade" rowspan="2" width="70" align="center" formatter="formatDash">Grade</th>
                            </tr>
                            <tr>
                                @foreach ($activeSizes as $i => $size)
                                    <th field="qty{{ $i }}" width="85" align="center" formatter="formatQty">{{ $size }}</th>
                                @endforeach
                            </tr>
                        </thead>
                    </table>
                ...
                {{-- TAB: Barang Diterima --}}
                <div id="tabPaneReceived" class="d-none">
                    <table id="dgReceived" class="easyui-datagrid" style="width:100%;height:480px"
                        url="{{ route('sisa-produksi.detail.list', $dt->popk) }}" method="get" pagination="true"
                        pageSize="50" pageList="[25,50,100,200,500]" rownumbers="false" singleSelect="true"
                        fitColumns="false" border="false"
                        data-options="
                            queryParams: {cr: '{{ $cr }}', tab: 'received'},
                            loadMsg: 'Memuat data...',
                            onLoadSuccess: onLoadTableGeneric,
                            onBeforeLoad: clearEmptyState,
                            onLoadError: onLoadTableError
                        ">
                        <thead>
                            <tr>
                                <th field="no" rowspan="2" width="50" align="center" formatter="formatNo">No.</th>
                                <th field="tglin" formatter="formatDatePlain" rowspan="2" width="110" align="center">Tanggal Diterima</th>
                                <th colspan="{{ count($activeSizes) }}" align="center">
                                    <strong>Size</strong>
                                    @if (!empty($dt->secsz)) ({{ $dt->secsz }}) @endif
                                </th>
                ...
                {{-- TAB: Barang Keluar --}}
                <div id="tabPaneKeluar" class="d-none">
                    <table id="dgKeluar" class="easyui-datagrid" style="width:100%;height:480px"
                        url="{{ route('sisa-produksi.detail.list', $dt->popk) }}" method="get" pagination="true"
                        pageSize="50" pageList="[25,50,100,200,500]" rownumbers="false" singleSelect="true"
                        fitColumns="false" border="false"
                        data-options="
                            queryParams: {cr: '{{ $cr }}', tab: 'keluar'},
                            loadMsg: 'Memuat data...',
                            onLoadSuccess: onLoadTableGeneric,
                            onBeforeLoad: clearEmptyState,
                            onLoadError: onLoadTableError
                        ...
                        <thead>
                            <tr>
                                <th field="keluar_status" rowspan="2" width="60" align="center" formatter="formatKeluarCheck">✓</th>
                                <th field="action" rowspan="2" width="70" align="center" formatter="formatActionKeluar">Aksi</th>
                                <th field="no" rowspan="2" width="50" align="center" formatter="formatNo">No.</th>
                                <th field="grade" rowspan="2" width="60" align="center" formatter="formatDash">Grade</th>
                                <th field="pcs" rowspan="2" width="90" align="center" formatter="formatTotal">Total<br>(Pcs)</th>
                                <th field="pcsk" rowspan="2" width="90" align="center" formatter="formatKeluar">Sudah<br>Keluar</th>
                                <th field="tglout" formatter="formatDatePlain" rowspan="2" width="105" align="center">Tanggal Keluar Terakhir</th>
                                <th field="keterangan" rowspan="2" width="180" align="center" formatter="formatDash">Keterangan</th>
                            </tr>
                            <tr></tr>
                        </thead>
                    </table>
...
@section('js_custom')
    <script>
        ...
        function formatActionPending(value, row, index) {
            const grade = row.grade;
            const hasGrade = grade !== null && grade !== undefined && String(grade).trim() !== '';
            if (!hasGrade) {
                return '<span class="dg-empty-cell">-</span>';
            ...
            return `
                <button type="button" class="dg-action-btn dg-check" title="Konfirmasi Terima dari Admin Finishing"
                    onclick="completeRow(${row.bjpk})">
    ...
    const loadedTabs = { pending: false, received: false, keluar: false };
    ...
    function switchSisaTab(tab) {
        ...
        $('#tabPanePending, #tabPaneReceived, #tabPaneKeluar').addClass('d-none');
        const paneMap = { pending: '#tabPanePending', received: '#tabPaneReceived', keluar: '#tabPaneKeluar' };
        $(paneMap[tab]).removeClass('d-none');
 
        const gridMap = { pending: '#dgPending', received: '#dgReceived', keluar: '#dgKeluar' };
        const gridSel = gridMap[tab];
 
        if (!loadedTabs[tab]) {
            $(gridSel).datagrid();
            loadedTabs[tab] = true;
        } else {
            $(gridSel).datagrid('reload');
        }
    }
 
    // ============================================================
    // FORMATTER khusus tab Barang Keluar -- checkbox-style indikator
    // + tombol Edit (Input Actual Keluar) HANYA kalau masih ada sisa
    // (pcs > pcsk).
    // ============================================================
    function formatKeluarCheck(value) {
        if (value === 'selesai') {
            return '<i class="fas fa-square-check text-success" style="font-size:16px;" title="Sudah keluar semua"></i>';
        }
        if (value === 'sebagian') {
            return '<i class="fas fa-square-minus text-warning" style="font-size:16px;" title="Sebagian sudah keluar"></i>';
        }
        return '<i class="far fa-square text-muted" style="font-size:16px;" title="Belum ada yang keluar -- menunggu"></i>';
    }
 
    function formatActionKeluar(value, row, index) {
        const hasBalance = Number(row.pcs) > Number(row.pcsk || 0);
        if (!hasBalance) {
            return '<span class="dg-empty-cell">-</span>';
        }
        return `
            <button type="button" class="dg-action-btn dg-edit" title="Input Actual Keluar"
                onclick="editKeluarByIndex(${index})">
                <i class="fas fa-pen-to-square"></i>
            </button>
        `;
    }
 
    function editKeluarByIndex(index) {
        const row = $('#dgKeluar').datagrid('getRows')[index];
        if (row) fillEditForm(row);
    }
 
    // ============================================================
    // onLoadSuccess -- pending TAMBAH update badge count, yang lain
    // reuse onLoadTable generik.
    // ============================================================
    function onLoadTablePending(data) {
        $('#countPending').text((data.rows || []).length ? data.total : 0);
        onLoadTableGeneric.call(this, data);
    }
 
    function onLoadTableGeneric(data) {
        const rows = data.rows || [];
        const gridId = this.id; // 'this' = elemen table yg trigger event
        if (!rows.length) {
            showEmptyStateFor(gridId, 'No Data Found', 'Belum ada data pada tab ini');
        } else {
            clearEmptyStateFor(gridId);
        }
    }
 
    function clearEmptyStateFor(gridId) {
        $('#' + gridId).datagrid('getPanel').find('.datagrid-view2 .easyui-empty-state').remove();
    }
 
    function showEmptyStateFor(gridId, title, subtitle) {
        let panel = $('#' + gridId).datagrid('getPanel');
        let body = panel.find('.datagrid-view2 .datagrid-body');
        panel.find('.easyui-empty-state').remove();
        body.append(`
            <div class="easyui-empty-state">
    ...
    // reloadTransferGrid() -- GANTI supaya reload grid tab yang AKTIF saja
    // (dipanggil setelah completeRow()/saveTransfer sukses).
    function reloadTransferGrid() {
        const gridMap = { pending: '#dgPending', received: '#dgReceived', keluar: '#dgKeluar' };
        const gridSel = gridMap[sisaActiveTab];
        if ($(gridSel).data('datagrid')) {
            $(gridSel).datagrid('reload');
        }
        // Pending count perlu selalu ikut update walau lagi di tab lain,
        // karena konfirmasi bisa mengurangi jumlah pending.
        if (sisaActiveTab !== 'pending' && loadedTabs.pending) {
