<!DOCTYPE html>
<html lang="id">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>PDF Tanda Terima</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            font-size: 12px;
        }

        .header,
        .footer {
            text-align: center;
        }

        table {
            width: 100%;
            border-collapse: collapse;
            margin-top: 2px;
        }

    

        .no-border {
            border: none;
        }


        .container {
            width: 100%;
            /* border-collapse: collapse; */
        }

        .left-column {
            width: 30%;
            vertical-align: top;
        }

        .right-column {
            width: 16%;
            vertical-align: top;
        }

        td {
            padding: 5px;
        }

        .judul-surat {
            text-align: center;
            font-size: 18px;
            font-weight: bold;
        }

        .separator {
            border-bottom: 1px solid black;
            width: 290px;
            /* Lebar garis, bisa disesuaikan */
            margin: 3px auto;
            /* Otomatis rata tengah */
        }

        .subjudul {
            text-align: center;
            font-size: 14px;
        }
    </style>
</head>

<body>

    <!-- Header Section -->
    <table class="header">
        <div><img src="{!! asset('public/css/images/morich.jpg') !!}" width="500" height="35" style="margin-top:3%;margin-left:3%;"></div>
        <div style="margin-top:-5%;margin-left:-89%;"><img src="{!! asset('public/css/images/morich.png') !!}" width="75" height="75"></div>
        <div style="margin-top:-7%;margin-left:1%;text-align:center;">
            <p style="font-size:13px;">JL. RAYA KARANGJATI KM. 25 DESA GEMBONGAN - BERGAS, SEMARANG - INDONESIA<br>
                PHONE : 0298 - 523523 (6 LINES) FAX. 0298 - 522095<br>
                www.morichindofashion.com</p>
        </div>
        <hr style="border: none;border-top: 2px solid black;margin-top:2%;">
        </hr>
    </table>
    <br>

    <table class="container">
        <tr>
            <td colspan="2" class="judul-surat">
               Tanda Terima Permintaan Barang
                <div class="separator"></div>
            </td>
        </tr>

        <tr>
            <td colspan="2" class="subjudul">
                <b>NO. {{ $dt_tt->nobukti }}</b>
            </td>
        </tr>
        <br>

        <tr>
            <td class="left-column">
                <table>
                    <tr>
                        <td>Tanggal : <strong>{{ (\Carbon\Carbon::parse($dt_tt->tgl)->format('d-m-Y')) }} </strong></td>
                    </tr>
                </table>
            </td>

            <td class="right-column">
                <table>
                    <tr>
                       <td>Untuk Departemen : <strong>{{ $dt_tt->penerima}} </strong></td>
                    </tr>
                </table>
            </td>
        </tr>
    </table>

    <table border="1">
        <tr>
            <th align="left">&nbsp; No</th>
            <th align="left">&nbsp; Keterangan</th>
            <th align="center">Qty</th>
        </tr>

        <tbody>
            @foreach($dt_ttdt as $index=>$dt)
            <tr>
                <td align="left">&nbsp; {{ ++$index }}</td>
                <td align="left">&nbsp; {{ ucwords($dt->brgnm) }}</td>
                <td align="center">{{ $dt->jumlah }} {{ $dt->satuan }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>

    <table class="no-border" style="font-size: 12px;">
        <tr>
            <td class="left-column">
                <table>
                    <tr>
                        <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Penerima </td>
                    </tr>
                    <br>
                    <br>
                    <br>
                    <tr>
                        <td>
                            __________________
                        </td>
                    </tr>
                </table>
            </td>

            <td class="right-column">
                <table>
                    <tr>
                       <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Pengirim </td>
                    </tr>
                    <br>
                    <br>
                    <br>
                    <tr>
                        <td>
                            __________________
                        </td>
                    </tr>
                </table>
            </td>
        </tr>
    </table>

</body>

</html>