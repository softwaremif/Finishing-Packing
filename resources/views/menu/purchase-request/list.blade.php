...
<script>
  ...
  $(function() {
    ...
    $('#dg-pr-mobile').datagrid('getPanel').find('div.datagrid-header').css('visibility', 'hidden');
    $('#dg-pr-mobile').datagrid('getPanel').find('div.datagrid-header').addClass('h-0');
    $('#dg-pr-mobile').datagrid('getPanel').css('border', 'none');
    $('#dg-pr-mobile').datagrid('getPanel').addClass('lines-no');
    $('#dg-pr-mobile').datagrid('getPanel').addClass('lines-no3');
  })
  ...
  function settingTableHeight(length) {
    ...
    if (length <= 6) {
      ...
      $('#dg-pr-mobile').datagrid('getPanel').find('div.datagrid-view2').css('height', windowHeight);
    } else {
      console.log("here auto");
      $('#dg-pr').datagrid('getPanel').find('div.datagrid-view2').css('height', 'auto');
      $('#dg-pr-mobile').datagrid('getPanel').find('div.datagrid-view2').css('height', 'auto');
    }
  }
  ...
  //   var userpklog = "{{ Session::get('userpk') }}";
  //   var postingButton = '';

  //   // if (row.posting == 1) {
  //   //     postingButton = '<li><a class="dropdown-item disabled">Posting</a></li>';
  //   // } else {
  //   //     postingButton = '<li><a class="dropdown-item dropdown-item-custom" id="btn-posting-' + (row.index || '') + '" onclick="KlikPosting(' + (row.prpk || 'null') + ');" style="color:#359DD9;">Posting</a></li>';
  //   // }

  //   // if(userpklog == row.userbuat){
  //   //   if (row.posting == null) {
  //   //       postingButton = '<li><a class="dropdown-item disabled">Posting</a></li>';
  //   //   } else {
  //   //       postingButton = '<li><a class="dropdown-item dropdown-item-custom" id="btn-posting-' + (row.index || '') + '" onclick="KlikPosting(' + (row.prpk || 'null') + ');" style="color:#359DD9;">Posting</a></li>';
  ...
  //     '<li><a class="dropdown-item dropdown-item-custom" id="btn-detail-' + (row.index || '') + '" onclick="KlikDetail(' + (row.prpk || 'null') + ');" style="color:#359DD9;">Detail PR</a></li>' +
  //     hr +
  //     postingButton +
  //     hr +
  //     '<li><a class="dropdown-item dropdown-item-custom" id="btn-print-' + (row.index || '') + '" onclick="KlikPrintPr(' + (row.prpk || 'null') + ');" style="color:#359DD9;">Print</a></li>' +
  //     '</ul>' +
  //     '</div>';
  // }
  ...
  function formatAttribute(value, row) {
    var hr = '<li><hr class="dropdown-divider mt-1 mb-1"></li>';
    var imageUrl = "{{ asset('public/css/images/More.png') }}";
    var userpklog = "{{ Session::get('userpk') }}";
    var postingButton = '';
    ...
    return '<div>' +
        ...
        '<ul id="dropdown-menu-action-' + (row.index || '') + '" class="dropdown-menu pt-1 pb-1">' +
        '<li><a class="dropdown-item dropdown-item-custom" id="btn-detail-' + (row.index || '') + '" onclick="KlikDetail(' + (row.prpk || 'null') + ');" style="color:#359DD9;">Detail PR</a></li>' +
        (postingButton ? hr + postingButton : '') +
        hr + // <--- tetap ditampilkan untuk pisah ke "Print"
        '<li><a class="dropdown-item dropdown-item-custom" id="btn-print-' + (row.index || '') + '" onclick="KlikPrintPr(' + (row.prpk || 'null') + ');" style="color:#359DD9;">Print</a></li>' +
        '</ul>' +
        '</div>';
}

  function KlikPosting(prpk){
    $('#prpk').val(prpk);
  ...
  function KlikDetail(prpk) {
    const baseUrl = "{{ url('/')}}";
    window.location.href = baseUrl + "/purchase-request/detail/" + prpk;
  }

  function AddPr() {
    $.ajax({
      type: "POST",
      url: "{{ route('get.lastpr') }}",
      data: {
      ...
      success: function(data) {
        console.log(data);
        const baseUrl = "{{ url('/')}}";
        window.location.href = baseUrl + "/purchase-request/add-pr/" + data.prpk;
      },
  ...
  function KlikPrintPr(prpk) {
    const baseUrl = "{{ url('/')}}";
    window.open(baseUrl + "/purchase-request/print/" + prpk, "_blank");
  }
</script>
@endsection
...
<div class="main-wrapper">
  <div class="container-fluid p-4">
    <div class="d-flex flex-column">
      <div class="p-2">
        <span class="txt-000000-700-24-28">List Purchase Request</span>
      </div>
      ...
      <div class="d-flex flex-wrap p-0 pb-2 gap-2 align-items-center">
        ...
        <button onclick="AddPr();" type="button" class="btn btn-dark" style="height:30px; line-height:30px; padding:0 10px;">&#10010; Purchase Request</button>
        ...
        <div class="ms-auto">
          <div id="sortlist-grup" class="p-1 pb-0 responsive">
            ...
            <button class="btn btn-sortlist dropdown-toggle-custom-sortlist pt-0" data-bs-toggle="dropdown" data-bs-target="#menu-sortlist" aria-expanded="true">
              ...
              <span id="Invdatedesc">Date PR (desc)</span>
              <span id="Invdateasc" class="d-none">Date PR (asc)</span>
            </button>
            <ul id="menu-sortlist" class="dropdown-menu dropdown-menu-end dropdown-menu-custom-global dropdown-menu-sortlist pt-0 pb-0">
              <li><a class="dropdown-item dropdown-item-custom-global dropdown-item-custom-global-start p-2" onclick="sortlistByDate(11)">Date PR (descending)</a></li>
              <li><a class="dropdown-item dropdown-item-custom-global dropdown-item-custom-global-end p-2" onclick="sortlistByDate(12)">Date PR (ascending)</a></li>
            </ul>
          </div>
        </div>
      ...
      <div class="d-none">
        <table id="dg-pr-shadow" class="easyui-datagrid" title="" style="width:100%;" align="center" toolbar="#tb" striped="false" pageSize="100" pageList="[100,200,300,500]" idField="prpk" pagination="true" rownumbers="false" multiple="true" collapsible="true" method="get"
          url="{{ route('api.get-pr') }}">
        </table>
      </div>


      <div class="p-0">
      <table id="dg-pr" class="easyui-datagrid" url="{{ route('api.get-pr')}}" title="" style="width:99%;" align="center" toolbar="#tb" 
      ...
      collapsible="true" method="get"
        ...
        <thead>
          <tr>
            ...
            <th field="tanggal" width="9%" styler="styler2">Tanggal PR</th>
            <th field="nopr" width="auto" styler="styler2">No PR</th>
            <th field="user" width="auto" styler="styler2">Pengirim</th>
            <th field="depnm" width="auto" styler="styler2">Penerima</th>
            <th field="totitem" width="auto" styler="styler2">Total Item</th>
            <!-- <th field="posting" width="auto" styler="styler2">Posting</th> -->
            <th field="act" width="3%" styler="styler3" formatter="formatAttribute">Act</th>
          </tr>
        </thead>
      </table>
      </div>
